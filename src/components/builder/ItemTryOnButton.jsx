...
export default function ItemTryOnButton({ item, profile, variant = "button", buttonRef }) {
  ...
  const handleSaveAsAvatar = async () => {
    setSaving(true);
    try {
      const res = await base44.functions.invoke('saveAvatarImage', { imageUrl });
      window.dispatchEvent(new CustomEvent('avatar-updated', { detail: { avatar_generated_url: res.data.file_url } }));
      setIsOpen(false);
    } catch (err) {
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-2 flex-col">
                    <div className="flex gap-2">
                      ...
                      <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleDownload(); }} className="flex-1 px-3 py-2 bg-white/10 hover:bg-white/20 text-white rounded-md text-sm inline-flex items-center justify-center gap-1">
                        <Download className="w-4 h-4" /> Download
                      </button>
                    </div>
                    <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleSaveAsAvatar(); }} disabled={saving} className="w-full px-3 py-2 bg-[#e8b820] hover:bg-[#d4a017] disabled:opacity-50 text-black font-semibold rounded-md text-sm inline-flex items-center justify-center gap-1">
